...
export default function Material({ bucket, prefix = '' }: Props) {
    const [files, setFiles] = useState<{ name: string; id?: string; updated_at?: string }[]>([])
    ...
    useEffect(() => {
        ...
        loadFiles()
        // opcional: volver a cargar al cambiar bucket/prefix
        return () => {
            cancelled = true
        }
    }, [bucket, prefix])
                    ...
                    {files.map((f) => (
                        <li key={f.name} className="flex items-center justify-between rounded border border-gray-200 p-2">
                            ...
                            <a
                                ...
                                href={
                                    supabase.storage.from(bucket).getPublicUrl(prefix ? `${prefix}/${f.name}` : f.name).data.publicUrl
                                }
                                target="_blank"
                                rel="noreferrer"
                            >
